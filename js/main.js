...
$('#setexample').on('change', function () {
    ...
    if (value == 1) {
        clearGraph();
        $('#svg-map').css({
            'backsground-image': 'url(' + "img/map1.png" + ')',
        ...
        $.getJSON("https://api.myjson.com/bins/grcnd", function (datad) {
        ...
        $.getJSON("https://api.myjson.com/bins/14e9d5", function (datad) {
            var importedData = datad;
        ...
        $.getJSON("https://api.myjson.com/bins/rcv7t", function (datad) {
            var importedData = datad;
...
$("#set-background-image").change(function (e) {
    e.stopPropagation();
    ...
    if (/^image/.test(files[0].type)) {
        ...
        reader.onload = function () {
            try {
                ...
                console.log("** Error setting Image: %s", exception);
                return;
            }
